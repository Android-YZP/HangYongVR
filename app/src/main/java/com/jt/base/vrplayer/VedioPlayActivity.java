...
public class VedioPlayActivity extends Activity {
		...
		private int mFov;
		private int mProjectionType = VedioPlayActivity.SNVR_PROJ_PLANE;
		private int mVideoSpliceFormat = VedioPlayActivity.SNVR_VIDEO_SPLICE_FMT_2D;
		private int mNavigationMode = VedioPlayActivity.SNVR_NAVIGATION_BOTH;
		private int mEyesMode = VedioPlayActivity.SNVR_SINGLE_EYES_MODE;
...
//        setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);//竖屏
			...
			mVideoView.setScale(VedioPlayActivity.SCALE_10);
		...
		private void initPlayMode() {
			...
			if (vedioode == VedioContants.TWO_D_VEDIO) {
				mEyesMode = VedioPlayActivity.SNVR_SINGLE_EYES_MODE;
				mProjectionType = VedioPlayActivity.SNVR_PROJ_PLANE;
			} else if (vedioode == VedioContants.ALL_VIEW_VEDIO) {
				mEyesMode = VedioPlayActivity.SNVR_SINGLE_EYES_MODE;
				mProjectionType = VedioPlayActivity.SNVR_PROJ_SPHERE;
			}
			mVideoView.setProjectionType(mProjectionType);
...
//                            mVideoView.setPlayFov(seekBar.getValue());
		...
		private void popSensorModeDialog(final MenuItem item) {
			...
			builder.setSingleChoiceItems(new String[]{"Sensor", "Touch",
					...
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int which) {
							if (VedioPlayActivity.SNVR_NAVIGATION_TOUCH == which) {
								item.setIcon(R.drawable.touch);
							} else if (VedioPlayActivity.SNVR_NAVIGATION_BOTH == which) {
								item.setIcon(R.drawable.both);
							} else if (VedioPlayActivity.SNVR_NAVIGATION_SENSOR == which) {
		...
		private void popEyesModeDialog(final MenuItem item) {
			...
			builder.setSingleChoiceItems(new String[]{"single eyes",
					...
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int which) {
							if (VedioPlayActivity.SNVR_SINGLE_EYES_MODE == which) {
								item.setIcon(R.drawable.vn_double_eye);
							} else if (VedioPlayActivity.SNVR_DOUBLE_EYES_MODE == which) {
		...
		private void popProjectionTypeDialog(final MenuItem item) {
			...
			builder.setSingleChoiceItems(
					...
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int which) {
							if (VedioPlayActivity.SNVR_PROJ_PLANE == which) {
								item.setIcon(R.drawable.vn_projection_plane);
							} else if (VedioPlayActivity.SNVR_PROJ_SPHERE == which) {
								item.setIcon(R.drawable.vn_projection_sphere);
							} else if (VedioPlayActivity.SNVR_PROJ_DOME == which) {
		...
		private void popDisplayModeDialog(final MenuItem item) {
			...
			builder.setSingleChoiceItems(new String[]{"2D", "3D Side By Side",
					...
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int which) {
							if (VedioPlayActivity.SNVR_VIDEO_SPLICE_FMT_2D == which) {
								item.setIcon(R.drawable.vn_display_mono);
							} else if (VedioPlayActivity.SNVR_VIDEO_SPLICE_FMT_3D_SBS == which) {
								item.setIcon(R.drawable.vn_display_side_by_side);
							} else if (VedioPlayActivity.SNVR_VIDEO_SPLICE_FMT_3D_OVU == which) {
		...
		private void showErrorDialog() {
			AlertDialog.Builder normalDialog =
					new AlertDialog.Builder(VedioPlayActivity.this, R.style.MyDialogStyle);
